...
class Toon(Avatar.Avatar, ToonHead):
    ...
    def putOnSuit(self, suitType, headType = 0, waiter = False, setDisplayName = True, rental = False):
        ...
        suit = Suit.Suit()
        dna = SuitDNA.SuitDNA()
        print ':Toon(putOnSuit): Loaded Cog'
        if rental == True:
            ...
            if SuitDNA.getSuitDept(deptIndex) == 's':
                ...
                self.deptName = TTLocalizer.Bossbot
        print ':Toon(putOnSuit): Got Cog Values'
        dna.newSuit(suitType)
        suit.setStyle(dna)
        print ':Toon(putOnSuit): Got cog appearance and assigned it to dna'
        suit.isDisguised = 1
        print ':Toon(putOnSuit): "disguised"'
        suit.generateSuit()
        suit.initializeDropShadow()
        suit.setPos(self.getPos())
        suit.setHpr(self.getHpr())
        print ':Toon(putOnSuit): Put cog in correct location'
    ...
    def takeOffSuit(self):
        ...
        print ':Toon(takeOffSuit): set nametag back'
        if self.isLocal():
            if hasattr(self, 'book'):
    ...
    def becomeChar(self, charType):
        ...
        char = Char.Char()
        dna = CharDNA.CharDNA()
        print ':Toon(becomeChar): Loaded Char'
        dna.newChar(CharDNA.charTypes[charType])
        print 'DNA OBTAINED'
        char.setDNA(dna)
        char.addActive()
        char.startEarTask()
        print ':Toon(becomeChar): Got char appearance and assigned it to dna'
        char.isTransformed = 1
        print ':Toon(becomeChar): "transformed"'
        #char.generateChar()
        char.initializeDropShadow()
        #print 'GENERATED AND INITIALIZEDDROPSHADOW'
        ...
        print ':Toon: Transformed!!'
        print self.char.style.getCharName()
